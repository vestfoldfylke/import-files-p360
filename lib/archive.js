...
const sendToDocument = async (barcodeData, file) => {
  ...
  if (barcodeData.docType === 'HOVED') {
    ...
    if (docData.Documents[0].Files.length > 0) {
      ...
      fileTitle = docData.Documents[0].Title.replace(/[^a-zA-Z0-9æøåÆØÅ. ]/gi, ' ').replace('  ', ' ').trim() // Frekk versjon som fikser det meste // replace(/[/\\?%*:|"<>-]/g, '-').trim() // SIF doesn't like ugly bugly characters in file titles
    }
  } else {
    fileTitle = 'Vedlegg'
  }
