...
const sendToUnreg = async (metadata) => {
  if (!(metadata.filename && metadata.note && metadata.ext && metadata.origin && metadata.filepath)) {
    throw new Error("Missing required properties in metadata (required: filename, note, ext, origin, filepath)")
  }
...
/**
 *
 * @param {import('./types').BarcodeData} barcodeData
 * @param {{ filePath: string, fileExt: string }} file
 * @returns {Promise<any>}
 */
const sendToDocument = async (barcodeData, file) => {
  ...
  if (barcodeData.docType === "HOVED") {
    ...
    if (docData.length === 0) {
      throw new Error(`Document with recno ${barcodeData.docRecno} does not exist...`)
    }

    if (docData[0].Files.length > 0) {
      fileTitle = "Vedlegg"
    } else {
      fileTitle = docData[0].Title.replace(/[^a-zA-Z0-9æøåÆØÅ. ]/gi, " ")
        .replace("  ", " ")
        .trim() // Frekk versjon som fikser det meste // replace(/[/\\?%*:|"<>-]/g, '-').trim() // SIF doesn't like ugly bugly characters in file titles
    }
  } else {
    fileTitle = "Vedlegg"
  }
