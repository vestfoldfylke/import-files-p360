...
const createStat = async (stat) => {
  if (!(stat.company && stat.description && stat.type)) {
    throw new Error("Stat must include properties: company, description, type")
  }

  if (!STATISTICS.URL || !STATISTICS.KEY) {
    throw new Error("STATISTICS_URL and STATISTICS_KEY must be set")
  }
