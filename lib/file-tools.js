...
const { existsSync, statSync, mkdirSync, renameSync, readdirSync, unlinkSync } = require("node:fs")
...
const moveToDir = (filePath, dir, newFileName) => {
  if (!filePath || !dir) {
    throw new Error('Missing required parameters "filePath" and/or "dir"')
  }
...
const deleteOldFiles = (dir, deleteAfterDays, ext) => {
  if (!existsSync(dir)) {
    mkdirSync(dir) // just to have it
  }

  const files = getFilesInDirWithMetadata(dir, ext)

  logger.info("Deleting files older than {DeleteAfterDays} days in {Dir} - checking {FileCount} number of files", deleteAfterDays, dir, files.length)
  const now = new Date()
  let deletedFiles = 0
  let sparedFiles = 0
  ...
  for (const file of files) {
    const daysOld = Math.floor((now.getTime() - file.modifiedTimestamp.getTime()) / (1000 * 60 * 60 * 24)) // Elapsed 24h periods, not calendar-day diff — off by up to a day, which is fine here
    if (daysOld > deleteAfterDays) {
      try {
        ...
        failedDeletions++
        logger.warn("Oh no! File should have died, but survived! Will live to see another day (or some minutes): {FileName}", file.fileName)
      }
    } else {
      sparedFiles++
    }
  ...
  logger.info("Finished deleting files. Deleted: {DeletedFiles} - Spared files: {SparedFiles} - Failed deletions: {FailedDeletions}", deletedFiles, sparedFiles, failedDeletions)
}
