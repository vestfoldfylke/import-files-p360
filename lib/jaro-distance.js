...
const jaroDistance = (s1, s2, options) => {
  ...
  for (let i = 0; i < len1; i++) {
    ...
    for (let j = Math.max(0, i - maxMatchingDistance); j < Math.min(len2, i + maxMatchingDistance + 1); j++) {
      ...
      if (s1[i] === s2[j] && hashS2[j] === 0) {
        if (options.verbose) {
          console.log(`i=${i} j=${j} - s1[i]=${s1[i]} matches s2[j]=${s2[j]}`)
        }
        ...
        if (options.verbose) {
          console.log(`i=${i} j=${j} - s1[i]=${s1[i]} does not match s2[j]=${s2[j]}`)
        }
      }
    }
  }
  ...
  for (let i = 0; i < len1; i++) {
    if (hashS1[i]) {
      // there is a match on this character in s2
      if (options.verbose) {
        console.log(`i=${i} point=${point} -  hashS1[i]=${hashS1[i]}`)
      }

      // Find the next matched character in second string
      while (hashS2[point] === 0) {
        // As long as no match in s2 - increment point
        if (options.verbose) {
          console.log(`i=${i} point=${point} - hashS2[point]=${hashS2[point]}`)
        }
      ...
      if (s1[i] !== s2[point++]) {
        if (options.verbose) {
          console.log(`i=${i} point=${point} - s1[i]=${s1[i]} not equal to s2[point++]=${s2[point - 1]} - incrementing transpositions`)
        }

        transpositions++
      } else {
        if (options.verbose) {
          console.log(`i=${i} point=${point} - s1[i]=${s1[i]} equal to s2[point++]=${s2[point - 1]} - transpositions remains untouched`)
        }
      }
    }
  }
  ...
  if (options.verbose) {
    console.log(`returning (${matches / len1}) + (${matches / len2}) + ((${matches - transpositions}) / ${matches}) / 3`)
  }
  ...
  if (!options.factorLengthDifference) {
    return jaroScore
  }
