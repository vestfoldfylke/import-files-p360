...
const getKompetansebevis = async (pdfData) => {
  ...
  if (!matchOnWords && !matchOnLines) {
    logger('info', ['Get-kompetansebevis', 'Fant ikke match på minst 7 av 9 ord, eller på tekstlinjer', matchWords, matchLines])
  ...
  if (fnrCheck.status === 'invalid') {
    logger('info', ['Get-kompetansebevis', `Fødselsnummeret som ble funnet er ikke gyldig, pga ${fnrCheck.reasons}, men vi har nok et kompetansebevis her`])
  ...
  try {
    ...
    logger('warn', ['Get-kompetansebevis', 'Failed when syncing privateperson, will wait for next run', error.response?.data || error.stack || error.toString()])
  ...
  if (jaroDistance(privatePersonCompareName, kompetansebevis.name) < 0.9) {
    logger('info', ['Get-kompetansebevis', `Navn fra SyncPrivatePerson: "${privatePersonCompareName}" lignet ikke nok på navn i pdf: "${kompetansebevis.name}"`])
  ...
  if (!schoolString) {
    ...
    if (schools.length === 1) kompetansebevis.school = schools[0]
      ...
      // logger('info' , ['Get-kompetansebevis', 'JaroSchools', schoolString, jaroSchools.map(s => { return { searchstring: s.searchstring, jaroScore: s.jaroScore }})])
      if (jaroSchools.length > 0) {
        kompetansebevis.school = jaroSchools[0]
      } else {
        logger('info', ['Get-kompetansebevis', `Fant ingen skole med strengen ${schoolString}`])
      }
    }
  }
