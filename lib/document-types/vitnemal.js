...
const searchSchools = getSchools()
  .filter((school) => school.countyNumber === COUNTY_NUMBER)
  .map((school) => {
    return { searchstring: school.fullName.replaceAll(" ", "").toLowerCase(), ...school }
  })
...
const getVitnemal = async (pdfData) => {
  ...
  if (!matchOnWords && !matchOnLines) {
    logger.info("Get-vitnemal - Fant ikke match på minst 7 av 9 ord, eller på tekstlinjer - MatchWords: {@MatchWords}", matchWords)
    return vitnemal
  }
  ...
  if (nameString.includes("Fødselsnummer:")) {
    nameString = nameString.substring(0, nameString.indexOf("Fødselsnummer:"))
  }
  ...
  if (fnrCheck.status === "invalid") {
    logger.info("Get-vitnemal - Fødselsnummeret som ble funnet er ikke gyldig, pga {@Reasons}, men vi har nok et vitnemål her", fnrCheck.reasons)
  ...
  try {
    ...
    logger.warn("Get-vitnemal - Failed when syncing privateperson, will wait for next run: {ErrorMessage}", formatError(error))
    vitnemal.waitForNextRun = true
    return vitnemal
  }
  // Her skal vi ha en privatperson
  if (!privatePerson) {
    throw new Error("Her skulle vi hatt privatePerson, men vi harn ikke???? Se på koden, den er rævva")
  }
  ...
  if (jaroDistance(privatePersonCompareName, vitnemal.name) < 0.9) {
    logger.info('Get-vitnemal - Navn fra SyncPrivatePerson: "{PrivatePersonName}" lignet ikke nok på navn i pdf: "{PdfName}"', privatePersonCompareName, vitnemal.name)
  ...
  if (!dateString) {
    ...
    if (potentialDate && new Date(potentialDate).toString() !== "Invalid Date" && new Date(potentialDate) < new Date()) {
      // Gyldig dato, og ikke en dato frem i tid
  ...
  if (!potentialSchoolString) {
    ...
    if (schools.length === 1) {
      ...
      const jaroSchools = searchSchools
        .map((s) => {
          return { jaroScore: jaroDistance(schoolString, s.searchstring), ...s }
        })
        .filter((jaroSchool) => jaroSchool.jaroScore > 0.81)
        .sort((a, b) => b.jaroScore - a.jaroScore)
      if (jaroSchools.length > 0) {
        vitnemal.school = jaroSchools[0]
      } else {
        logger.info("Get-vitnemal - Fant ingen skole med strengen {SchoolString}", schoolString)
      }
    }
  }
