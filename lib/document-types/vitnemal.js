...
const getVitnemal = async (pdfData) => {
  ...
  if (!matchOnWords) {
    logger('info', ["Fant ikke match på minst 7 av 9 ord", matchWords])
    return vitnemal
  }
  ...
  if (!vitnemalId) {
    logger('info', ["Fant ingen vitnemålsId"])
    return vitnemal
  }
  ...
  if (fnrCheck.status === 'invalid') {
    logger('warn', [`Fødselsnummeret som ble funnet er ikke gyldig, pga ${fnrCheck.reasons}, men vi har nok et vitnemål her`])
  ...
  for (const ssnLine of ssnLines) {
    ...
    if (ssn !== vitnemal.ssn) {
      logger('warn', [`Fant flere enn ett fødselsnummer i pdf-en, vi har nok flere vitnemål her`])
  ...
  if (jaroDistance(privatePersonCompareName, vitnemal.name) < 0.9) {
    logger('warn', [`Navn fra SyncPrivatePerson: "${privatePersonCompareName}" lignet ikke nok på navn i pdf: "${vitnemal.name}"`])
    vitnemal.message = 'Roboten synes ikke navnet i vitnemålet og navnet i folkeregisteret stemte overens...'
    return vitnemal
  }
  ...
  if (!schoolString) {
    ...
    if (schools.length === 1) school = schools[0]
    else {
