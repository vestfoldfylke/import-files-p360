...
const getProbableTitle = (textLines, knownTitles, zipcodes) => {
  ...
  for (const line of firstLines) {
    ...
    if (cleanedLine.length < 50 && ['vei', 'gate', 'veg', 'sving', 'krok', 'skog', 'byen'].some(substr => cleanedLine.includes(substr)) && lastCharsHaveNumber) { // Adresser burde være mindre enn 50 tegn, inneholde et eller annet vei-ord, og ha et tall i en av de to siste tegnene
    ...
    if (cleanedLine.includes("unntattoffentlighet") || line.includes('offl.') || superCleanedLine.includes('unntattoff') || cleanedLine.includes("unntoffentlighet") || cleanedLine.includes('offentleglova')) {
