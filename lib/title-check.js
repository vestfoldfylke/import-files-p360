...
const getProbableTitle = (textLines, knownTitles) => {
  ...
  for (const line of firstLines) {
    ...
    */
    ...
    if (lineWords.every(word => startsWithUppercaseAndRestIsLowercase(word))) {
      logger('info', ['Every word in the line starts with uppercase and rest is lowercase, probably a name', line, lineWords])
      continue
    }
  ...
  return getProbableTitle(textLines.slice(9, textLines.length), knownTitles)
}

/*

let testLines = ["HJFUD", " X i / hfu", "Legeerklæring", "Arbidsavtale", "$&/$/()"]
...
*/
/*
const str1 = "Helseerklæring for skoleskyss i Vestfold"
const str2 = "HELSEERKLÆRING for akoleskyø I Vestfokl"
