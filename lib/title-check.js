...
const getProbableTitle = (textLines, knownTitles, zipcodes) => {
  ...
  for (const line of firstLines) {
    ...
    if (cleanedLine.length < 50 && ['vei', 'gate', 'veg', 'sving', 'krok'].some(substr => cleanedLine.includes(substr)) && lastCharsHaveNumber) { // Adresser burde være mindre enn 50 tegn, inneholde et eller annet vei-ord, og ha et tall i en av de to siste tegnene
    ...
    if (lineWords.length > 1 && lineWords.every(word => startsWithUppercaseAndRestIsLowercase(word))) {
      logger('info', ['At least two words, and every word in the line starts with uppercase and rest is lowercase, probably a name', line, lineWords])
      continue
    }
...
*/
/*
const str1 = "Refusjon skyss utplassering ID 296395"
const str2 = "Refusjon skyss utplassering"
