...
const getUnregAdUser = async (email) => {
  ...
  logger.info('Looking for {Email} in OU {Ou}', email, UNREGISTERED.AD_MAIN_COUNTY_OU)
  let adUser = await getAdUser(adUserOptions)
  if (adUser && Array.isArray(adUser)) {
    logger.warn('Found several users in AD ({Ou}) with email {Email}, returning the first one - but should probably be checked', UNREGISTERED.AD_MAIN_COUNTY_OU, email)
    return adUser[0]
  }

  if (adUser) {
    logger.info('Found user {Email} in OU {Ou}', email, UNREGISTERED.AD_MAIN_COUNTY_OU)
    return adUser
  }

  adUserOptions.countyOU = 'VTFK'
  
  logger.info('Looking for {Email} in OU {Ou}', email, 'VTFK')
  
  adUser = await getAdUser(adUserOptions)
  if (adUser && Array.isArray(adUser)) {
    logger.warn('Found several users in AD (VTFK) with email {Email}, returning the first one - but should probably be checked', email)
    return adUser[0]
  }

  if (adUser) {
    logger.info('Found user {Email} in OU {Ou}', email, 'VTFK')
    return adUser
  }
