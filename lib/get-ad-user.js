...
module.exports.getAdUser = async (adUserOptions) => {
  const command = `Get-ADUser -SearchBase 'OU=${adUserOptions.usersOU},OU=USERS,OU=${adUserOptions.countyOU},DC=top,DC=no' -Filter '${adUserOptions.filter}' -Properties ${adUserOptions.properties}`
