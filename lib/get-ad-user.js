...
module.exports.getAdUser = async (adUserOptions) => {
  const command = `Get-ADUser -SearchBase 'OU=${adUserOptions.usersOU},OU=USERS,OU=${adUserOptions.countyOU},DC=top,DC=no' -Server ${adUserOptions.domain} -Filter '${adUserOptions.filter}' -Properties ${adUserOptions.properties} | Select-Object (${adUserOptions.properties}`
