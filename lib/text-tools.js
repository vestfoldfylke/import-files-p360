...
const hasTextElements = (textElements, matchElements, options) => {
  const {
    matchThreshold = matchElements.length,
    jaroThreshold = 1,
    jaroCaseSensitive = true,
    verboseJaro = false
  } = options ?? {}
  ...
  for (const matchElement of matchElements) {
    ...
    if (jaroThreshold !== 1) { // One equals complete match (100% identical strings)
      const match = textElements.some(textElement => {
        ...
        if (verboseJaro) console.log(`jaroDistance(${matchElement}, ${textElement}) = ${jwScore}`)
