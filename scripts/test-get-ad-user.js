(async () => {
  ...
  const psProbe = spawnSync('powershell.exe', [
  ...
  const modProbe = spawnSync('powershell.exe', [
    '-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass',
    '-Command', 'if (Get-Module -ListAvailable ActiveDirectory) { "OK" } else { exit 1 }'
  ], { encoding: 'utf-8', windowsHide: true })
